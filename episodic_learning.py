...
def create_batch_within_episode(episode):
    updated_episode = {
        'x': episode["xs"] + episode["xq"],
        'y_label': episode["ys_label"] + episode["yq_label"],
        'labels': episode["labels"],
        'y': episode["ys"] + episode["yq"]
    }
    batch_size = max(1, 128 // len(updated_episode['x']))
    for bidx in range(0, len(updated_episode['x']), batch_size):
        yield {
                'xs': updated_episode["x"],
                'xq': updated_episode["x"][bidx:bidx+batch_size],
                'ys_label': updated_episode["y_label"],
                'yq_label': updated_episode["y_label"][bidx:bidx + batch_size],
                'labels': updated_episode["labels"],
                'ys': updated_episode["y"],
                'yq': updated_episode["y"][bidx:bidx + batch_size]
        }
...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser("Run episodic learning with single set")
